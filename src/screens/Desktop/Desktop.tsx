...
const projectCards: ProjectCardData[] = [
  ...
  },
  {
    ...
    technologies: [
      ...
      { name: "TypeScript", icon: "/icons/typescript-svgrepo-com.svg", noFilter: true },
  ...
  {
    title: { en: "Data Analytics Tool", fr: "Outil d'Analyse de Données" },
    description: {
      en: "Data processing and visualization tool for business intelligence, featuring real-time dashboards and automated reporting.",
      fr: "Outil de traitement et visualisation de données pour l'intelligence d'affaires, avec tableaux de bord en temps réel et rapports automatisés."
    },
    features: {
      en: ["Data Processing", "Visualization", "Automation"],
      fr: ["Traitement de Données", "Visualisation", "Automatisation"]
    },
    technologies: [
      { name: "Python", icon: "/icons/python.svg" },
      { name: "NodeJS", icon: "/icons/nodejs.svg" }
...
const ProjectCard = ({ card, t, language, onSeeMore }: { card: ProjectCardData; t: any; language: 'en' | 'fr'; onSeeMore?: () => void }) => (
      ...
      <div className="flex items-center justify-center gap-2 mb-4">
        ...
        >
          <img
            ...
            loading="lazy"
          />
          <span className="absolute left-full top-1/2 -translate-y-1/2 ml-0.5
      ...
      <div className="flex justify-between items-start mt-2.5 mb-3">
        ...
        <div className="flex flex-col gap-2 mr-10 font-['Avenir-Heavy'] text-sm text-[#393a3d] dark:text-[#efefef]">
          {card.technologies.map((tech, idx) => (
            <div key={idx} className="inline-flex h-[23px] items-center gap-1.5 px-3 py-1 bg-[#dbdbdb] dark:bg-[#393a3d] rounded-full">
              <img src={tech.icon} alt={tech.name} className={`w-[15px] h-[15px] min-w-[15px] min-h-[15px] ${tech.noFilter ? '' : '[filter:brightness(0)_saturate(100%)_invert(22%)_sepia(5%)_saturate(500%)_hue-rotate(182deg)_brightness(95%)_contrast(90%)] dark:[filter:brightness(0)_saturate(100%)_invert(100%)]'}`} loading="lazy" />
              <span>{tech.name}</span>
            </div>
          ))}
        </div>
            ...
            <div className="flex justify-end items-center gap-3 mb-8">
              ...
              >
                <img
                  ...
                  alt={language === 'en' ? 'Switch to French' : 'Switch to English'}
                  src={language === 'en' ? '/icons/fr_flag.svg' : '/icons/us_flag.svg'}
                  loading="lazy"
                />
              </button>
              <button
            ...
            <div className="w-full flex flex-col lg:flex-row lg:items-start lg:justify-between">
              <div className="w-full lg:w-[50%] px-4 md:px-8 lg:pl-20 xl:pl-32 pt-6 lg:pt-0 flex-shrink-0 flex flex-col items-center lg:items-start">
                ...
                <nav className="flex flex-wrap gap-3 mb-20 justify-center lg:justify-start">
                  <a href="https://github.com/Romain-Brgn" target="_blank" rel="noopener noreferrer"
                    ...
                    <img className="w-6 h-6 transition-all duration-150 [filter:brightness(0)_saturate(100%)_invert(22%)_sepia(5%)_saturate(500%)_hue-rotate(182deg)_brightness(95%)_contrast(90%)] dark:[filter:brightness(0)_saturate(100%)_invert(90%)] group-hover:[filter:brightness(0)_saturate(100%)_invert(100%)] dark:group-hover:[filter:brightness(0)_saturate(100%)_invert(22%)_sepia(5%)_saturate(500%)_hue-rotate(182deg)_brightness(95%)_contrast(90%)]" alt="GitHub" src="/icons/Property_1=Github.svg" loading="lazy" />
                    <span className="font-['Avenir-Heavy'] text-[#393a3d] dark:text-[#dbdbdb] text-sm md:text-base tracking-[0.80px] transition-colors duration-150 group-hover:text-white dark:group-hover:text-[#393a3d]">{t.socialButtons.github}</span>
                  </a>
                  <a href="https://www.linkedin.com/in/romain-bourgin/" target="_blank" rel="noopener noreferrer"
                    className="inline-flex items-center justify-center gap-2.5 px-3 py-2.5 bg-white dark:bg-[#2a2b2e] rounded-[25px] border border-solid border-[#dbdbdb] dark:border-[#4a4b4f] cursor-pointer transition-all duration-150 hover:bg-[#393a3d] dark:hover:bg-[#efefef] group">
                    <img className="w-6 h-6 transition-all duration-150 [filter:brightness(0)_saturate(100%)_invert(22%)_sepia(5%)_saturate(500%)_hue-rotate(182deg)_brightness(95%)_contrast(90%)] dark:[filter:brightness(0)_saturate(100%)_invert(90%)] group-hover:[filter:brightness(0)_saturate(100%)_invert(100%)] dark:group-hover:[filter:brightness(0)_saturate(100%)_invert(22%)_sepia(5%)_saturate(500%)_hue-rotate(182deg)_brightness(95%)_contrast(90%)]" alt="LinkedIn" src="/icons/Property_1=LinkedIn.svg" loading="lazy" />
                    <span className="font-['Avenir-Heavy'] text-[#393a3d] dark:text-[#dbdbdb] text-sm md:text-base tracking-[0.80px] transition-colors duration-150 group-hover:text-white dark:group-hover:text-[#393a3d]">{t.socialButtons.linkedin}</span>
                  </a>
                  <button onClick={() => handleSocialClick("Mail")}
                    className="inline-flex items-center justify-center gap-2.5 px-3 py-2.5 bg-white dark:bg-[#2a2b2e] rounded-[25px] border border-solid border-[#dbdbdb] dark:border-[#4a4b4f] cursor-pointer transition-all duration-150 hover:bg-[#393a3d] dark:hover:bg-[#efefef] group">
                    <img className="w-6 h-6 transition-all duration-150 [filter:brightness(0)_saturate(100%)_invert(22%)_sepia(5%)_saturate(500%)_hue-rotate(182deg)_brightness(95%)_contrast(90%)] dark:[filter:brightness(0)_saturate(100%)_invert(90%)] group-hover:[filter:brightness(0)_saturate(100%)_invert(100%)] dark:group-hover:[filter:brightness(0)_saturate(100%)_invert(22%)_sepia(5%)_saturate(500%)_hue-rotate(182deg)_brightness(95%)_contrast(90%)]" alt="Email" src="/icons/Property_1=Mail.svg" loading="lazy" />
                    <span className="font-['Avenir-Heavy'] text-[#393a3d] dark:text-[#dbdbdb] text-sm md:text-base tracking-[0.80px] transition-colors duration-150 group-hover:text-white dark:group-hover:text-[#393a3d]">{t.socialButtons.mail}</span>
                  </button>
                  <button onClick={() => handleSocialClick("Phone")}
                    className="inline-flex items-center justify-center gap-2.5 px-3 py-2.5 bg-white dark:bg-[#2a2b2e] rounded-[25px] border border-solid border-[#dbdbdb] dark:border-[#4a4b4f] cursor-pointer transition-all duration-150 hover:bg-[#393a3d] dark:hover:bg-[#efefef] group">
                    <img className="w-6 h-6 transition-all duration-150 [filter:brightness(0)_saturate(100%)_invert(22%)_sepia(5%)_saturate(500%)_hue-rotate(182deg)_brightness(95%)_contrast(90%)] dark:[filter:brightness(0)_saturate(100%)_invert(90%)] group-hover:[filter:brightness(0)_saturate(100%)_invert(100%)] dark:group-hover:[filter:brightness(0)_saturate(100%)_invert(22%)_sepia(5%)_saturate(500%)_hue-rotate(182deg)_brightness(95%)_contrast(90%)]" alt="Phone" src="/icons/Property_1=Phone.svg" loading="lazy" />
                    <span className="font-['Avenir-Heavy'] text-[#393a3d] dark:text-[#dbdbdb] text-sm md:text-base tracking-[0.80px] transition-colors duration-150 group-hover:text-white dark:group-hover:text-[#393a3d]">{t.socialButtons.phone}</span>
                  </button>
                </nav>
                ...
                <p className="font-['Avenir-Regular'] font-bold text-base md:text-lg lg:text-xl leading-relaxed text-[#393a3d] dark:text-[#dbdbdb] mb-10 max-w-2xl text-center lg:text-left">
                  ...
                  <span className="text-[#393a3d] dark:text-[#efefef] font-['Avenir-Black']">{t.hero.part2}</span>
                  <span>{t.hero.part3}</span>
                  <span className="text-[#393a3d] dark:text-[#efefef] font-['Avenir-Black']">{t.hero.part4}</span>
                  <span>{t.hero.part5}</span>
                  <span className="text-[#393a3d] dark:text-[#efefef] font-['Avenir-Black']">{t.hero.part6}</span>
                </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {projectCards.map((card, idx) => (
                <ProjectCard
                  ...
                  t={t}
                  language={language}
                  onSeeMore={card.isCurrentSite
                    ? () => showToast(language === 'en' ? 'You are on this portfolio!' : 'Vous êtes sur ce portfolio !')
                    : undefined
                  }
                />
              ))}
          ...
          <section className="mt-16 md:mt-20">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 lg:items-start">
              <div className="space-y-4 lg:w-full">
                {[
                  { icon: "M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4", text: t.lookingFor.school },
                  { icon: "M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z", text: t.lookingFor.degree },
                  { icon: "M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z", text: t.lookingFor.contractType },
                  { icon: "M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z", text: t.lookingFor.duration },
                  { icon: "M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z", text: t.lookingFor.rhythm },
                  { icon: "M13 10V3L4 14h7v7l9-11h-7z", text: t.lookingFor.ready },
                ].map((item, idx) => (
                  <div key={idx} className="bg-white/60 dark:bg-[#2a2b2e]/60 backdrop-blur-sm rounded-lg p-5 border-l-4 border-[#393a3d] dark:border-[#dbdbdb] transition-all duration-300 hover:bg-white/90 dark:hover:bg-[#2a2b2e]/90">
                    <div className="flex items-start gap-3">
                      <svg className="w-5 h-5 mt-0.5 flex-shrink-0 text-[#393a3d] dark:text-[#dbdbdb]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={item.icon} />
                      </svg>
                      <p className="font-['Avenir-Heavy'] text-[#393a3d] dark:text-[#efefef] text-sm md:text-base">{item.text}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="flex flex-col justify-center items-center lg:items-start space-y-6 lg:w-full">
                <div className="w-full max-w-md">
                  <div className="bg-[#393a3d] dark:bg-[#2a2b2e] rounded-xl p-8 shadow-xl border border-[#4a4b4f]">
                    <div className="space-y-4">
                      ...
                      <button
                        onClick={() => window.open('https://www.epsi.fr/programmes/bachelor-developpeur-ia', '_blank')}
                      ...
                      >
                        ...
                        <svg className="w-5 h-5 text-[#393a3d] transition-transform group-hover:translate-x-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div className="hidden lg:flex flex-col gap-3 text-center lg:text-left">
                  {[
                    language === 'en' ? 'School recognized by the State' : 'École reconnue par l\'État',
                    language === 'en' ? 'Diploma recognized at Bac+3 level' : 'Diplôme reconnu niveau Bac+3',
                    language === 'en' ? "Option to pursue a master's degree" : "Poursuite en Master possible",
                  ].map((text, idx) => (
                    <div key={idx} className="flex items-center gap-2 text-[#393a3d] dark:text-[#dbdbdb]">
                      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                        <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                      </svg>
                      <span className="font-['Avenir-Heavy'] text-sm">{text}</span>
                    </div>
                  ))}
                </div>
              </div>
